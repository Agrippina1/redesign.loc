...
function ShowForm(param)
{
        ...
        if (ignor) var form_ans = "<div style='background-color: #FDE5DD; border: 1px solid #ff0000; margin-bottom: 2em; padding: 1em;'>»звините, ¬ы внесены в игнор-лист автора дневника и не можете писать ответы.</div>";
        else {
            var form_ans = "<fieldset style='width: 710px; border: 1px solid #DFDFDF;'><legend>“екст ответа</legend>"
                + "<form id='answer_form'>"
                + "<textarea id='ans_text' cols='50' rows='5' name='ans_text'></textarea>"
                + "<input type='hidden' name='id' value='" + id + "'>"
                + "<div><input type='button' value='ќтправить' class='form_button' onClick='submitAnswer(" + id + ")'></div>"
                + "</form>"
                + "";
        }
        CloseAnswerForms();
...
function SubansToggle(id,mode)
{
	$(document).ready(function()
	{					 	
		if (mode=='yes') $("#ans_subp_"+id).html('<a href="#" onclick="return Subans(\''+id+'\',\'no\');"><img src="pics/com_s.png" width="16" height="16" border="0" alt="уведомл€ть об ответах" title="уведомл€ть об ответах" /></a>');
...
function DelAns(id)
{
	if (confirm("¬ы действительно хотите удалить сообщение?")) 
	{
		$(document).ready(function(){
...
function DelPost(id)
{
	if (confirm("¬ы действительно хотите удалить пост и все ответы к нему?")) 
	{
		$(document).ready(function(){
